...
new class extends Component
{
    ...
    public function mount(): void
    {
        $firstFiling = \App\Models\Filing::query()
            ...
            ->whereHas('filingType', fn ($query) => $query->where('code', '1040-NR'))
            ->with('taxYear')
    ...
    public function with(): array
    {
        ...
        if ($filing) {
            ...
            $summary = $service->getForm1040NrSummary($user, $year);
        }
...
<div class="space-y-6">
    <div>
        <flux:heading size="lg">{{ __('Form 1040-NR Summary') }}</flux:heading>
        <flux:subheading>{{ __('Nonresident alien income and deductions grouped by line item.') }}</flux:subheading>
    </div>

    <div class="grid gap-4 md:grid-cols-3">
        <flux:select wire:model.live="filingId" label="{{ __('Form 1040-NR Filing') }}" placeholder="{{ __('Select a filing') }}">
    ...
    @if (! $filing)
        <div class="rounded-xl border border-dashed border-zinc-300 bg-zinc-50 p-6 text-center text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
            {{ __('No Form 1040-NR filings found. Create a Form 1040-NR filing in your tax year filings to get started.') }}
        </div>
    @else
        <div class="grid gap-4 md:grid-cols-3">
        ...
        <div class="rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
            <strong>{{ __('Note') }}:</strong> {{ __('All amounts are shown in USD and grouped by 1040-NR line item mapping.') }}
        </div>
    @endif
</div>