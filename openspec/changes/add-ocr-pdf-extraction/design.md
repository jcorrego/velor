...
The OCR capability should also be reusable for legal document ingestion so documents become searchable.
...
## Decisions
- Decision: Use Tesseract for OCR fallback extraction when text extraction yields no transactions.
- Decision: Use the tesseract-ocr-for-php wrapper for integration.
...
- OCR adds processing time and system dependencies (Tesseract binary).

## Migration Plan
- Add the tesseract-ocr-for-php wrapper and configuration.
