# Specification: Tax Form Mapping

## Overview
Tax form mapping connects user-defined financial categories to tax form line items, enabling automatic computation of tax-reportable amounts across jurisdictions.
...
## Data Integrity
- Category names MUST exist in the finance database
- Tax form codes MUST be in the supported forms list
- Line item numbers MUST be valid for the specified tax form
- Jurisdictions MUST be supported (Spain, USA, Colombia, etc.)
- Amounts computed from mappings MUST NOT include unmapped transactions

## Error Handling
- Mapping a non-existent category SHALL raise CategoryNotFoundException
- Using an unsupported tax form code SHALL raise UnsupportedTaxFormException
- Invalid line item number for tax form SHALL raise InvalidLineItemException
- Jurisdiction mismatch between category and mapping SHALL raise JurisdictionMismatchException
